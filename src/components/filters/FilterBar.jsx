    ...
    <div className="filter-bar">
        ...
        <div className="filter-bar__tags">
          ...
          {moments.map(m => (
            <Tag
              key={m}
              type={m}
              onClick={() => onMomentChange(selectedMoment === m ? null : m)}
              active={selectedMoment === m}
            >
              {MOMENT_LABELS[m] || m}
            </Tag>
          ))}
        ...
        <div className="filter-bar__group">
          ...
          <div className="filter-bar__tags">
            ...
            {formats.map(f => (
              <Tag
                key={f}
                onClick={() => onFormatChange(selectedFormat === f ? null : f)}
      ...
      {subniches.length > 0 && (
        <div className="filter-bar__group">
          ...
          <div className="filter-bar__tags">
            <Tag
              onClick={() => onSubnicheChange(null)}
              active={!selectedSubniche}
            >
              Todos
            </Tag>
            {subniches.map(s => (
              <Tag
                key={s}
                onClick={() => onSubnicheChange(selectedSubniche === s ? null : s)}
                active={selectedSubniche === s}
              >
                {s}
              </Tag>
            ))}
